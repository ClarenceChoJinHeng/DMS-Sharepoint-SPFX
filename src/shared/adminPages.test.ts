...
describe("the page as designed", () => {
  ...
  it("puts CRS Term Abbreviations under Folder Management, where the client moved it", () => {
    const folders = CARDS.filter((c) => c.key === "folders")[0];
    expect(folders.links.map((l) => l.label)).toContain("CRS Term Abbreviations");
  });
...
describe("resolveLink — the collisions that would send an admin to the wrong page", () => {
  it("sends all three Folder Management rows to Folder Administration, not Folder Access", () => {
    // The bug this pattern exists to prevent: a bare /folder/i claims Folder-Access.aspx, so
    // "Folder Reconciliations" opens a permissions screen.
    for (const key of ["abbreviations", "structure", "reconciliation"]) {
      const t = resolveLink(linkFor(key), SITE);
      expect(t.state).toBe("resolved");
      if (t.state !== "missing") expect(t.url).toContain("Folder-Administration.aspx");
    }
  });
  ...
  it("gives every link a DISTINCT page, except the three sharing Folder Administration", () => {
    const all = resolveAll(SITE);
    const distinct: Record<string, true> = {};
    ...
    // 3 rows share Folder Administration; every other link must be somewhere of its own.
    expect(Object.keys(distinct).length).toBe(allLinks().length - 2);
  });
});
...
describe("the tab hash", () => {
  it("is appended only for a link that names a tab", () => {
    const recon = resolveLink(linkFor("reconciliation"), SITE);
    const access = resolveLink(linkFor("folderAccess"), SITE);
    if (recon.state !== "missing") expect(recon.url).toContain("#tab=reconciliation");
    if (access.state !== "missing") expect(access.url).not.toContain("#tab=");
  });

  it("survives on an override too — the tab belongs to the link, not the page", () => {
    const t = resolveLink(linkFor("structure"), SITE, "/sites/X/SitePages/FA.aspx");
    if (t.state !== "missing") expect(t.url).toBe("/sites/X/SitePages/FA.aspx#tab=structure");
  });

  it("gives the three shared rows three DIFFERENT tabs", () => {
    const tabs = ["abbreviations", "structure", "reconciliation"].map((k) => linkFor(k).tab);
    const distinct: Record<string, true> = {};
    for (const t of tabs) distinct[String(t)] = true;
    expect(Object.keys(distinct).length).toBe(3);
  });
});
...
describe("suggestedPageName", () => {
  ...
  it("names the SHARED page for a row that is only a tab of one", () => {
    // Suggesting "Folder-Reconciliations.aspx" would have an admin create a page the pattern can
    // never match, leaving the row dead after they did exactly as they were told.
    expect(suggestedPageName(linkFor("reconciliation"))).toBe("Folder-Administration.aspx");
    expect(suggestedPageName(linkFor("abbreviations"))).toBe("Folder-Administration.aspx");
  });
